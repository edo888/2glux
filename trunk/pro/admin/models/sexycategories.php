...
class SexypollingModelSexycategories extends JModel {
var $_data, $_total, $_pagination, $_filter, $_total_sql;
    ...
    function __construct() {
    	...
    	$this->_filter->filter_order		= $mainframe->getUserStateFromRequest( 'com_sexypolling'.'filter_order',		'filter_order',		'c.id',	'cmd' );
    	...
    	if (!in_array($this->_filter->filter_order, array('c.name','c.id','c.published','num_polls'))) {
    		$this->_filter->filter_order = 'c.id';
    	}
    ...
    function _buildQuery() {
    	...
    	if ($this->_filter->search != '') {
    		...
    		$where[] = 'LOWER(c.name) LIKE '.$this->_db->Quote( '%'.$this->_db->getEscaped( $this->_filter->search, true ).'%', false );
    	}
    	...
    	$query = 'SELECT c.*,COUNT(sp.id) num_polls '
            . ' FROM #__sexy_categories c '
            . ' LEFT JOIN #__sexy_polls sp '
            . ' ON sp.id_category = c.id '
            . ' GROUP BY c.id';
