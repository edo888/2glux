...
class plgSystemSexypolling extends JPlugin {
    ...
    function onAfterRender() {
      ...
      //add scripts
	  if(preg_match('/(\[sexypolling id|category="([0-9]+)"\])/s',$content))
      ...
      //plugin 
      $c = preg_replace_callback('/(\[sexypolling id="([0-9]+)"\])/s',array($this, 'sp_make_poll'),$content);
      preg_match_all('/(\[sexypolling id="([0-9]+)"\])/s',$content,$m);
      ...
      //plugin 
      $c = preg_replace_callback('/(\[sexypolling category="([0-9]+)"\])/s',array($this, 'sp_make_poll_category'),$c);
      preg_match_all('/(\[sexypolling category="([0-9]+)"\])/s',$content,$m);
